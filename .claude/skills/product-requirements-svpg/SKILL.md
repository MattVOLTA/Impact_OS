...
4. **Feasibility risk**: Can we build it with time/technology/skills available?
...
1. **Understand your context** through targeted questions (asked sequentially):
...
2. **Challenge solution-centric framing** (one probe at a time):
...
3. **Guide collaborative exploration** (iteratively):
...
4. **Deliver structured outputs** (only after gathering sufficient context):
...
### Frameworks
- `frameworks/four-risks.md` - Detailed guidance on assessing each risk type
...
- `templates/opportunity-assessment.md` - Format for evaluating opportunities
