/**
 * Programs Section - Contact Detail
 *
 * Shows programs this contact is enrolled in.
 * Same pattern as company version.
 */
...
import { EnrollContactInProgramModal } from './enroll-contact-in-program-modal'
import { EditEnrollmentDatesModal } from './edit-enrollment-dates-modal'
import { unenrollContactAction } from '../../../programs/actions'
...
export function ProgramsSection({ contactId, enrollments }: ProgramsSectionProps) {
  ...
  const handleUnenrollConfirm = async () => {
    ...
    const result = await unenrollContactAction(selectedEnrollment.program_id, contactId)
    if (result.success) {
        ...
        <CardContent>
          {enrollments.length === 0 ? (
            <div className="text-center py-8 text-sm text-muted-foreground">
              ...
              <br />
              Click "Enroll" to add this contact to a program.
            </div>
          ) : (
          ...
          <AlertDialog open={unenrollDialogOpen} onOpenChange={setUnenrollDialogOpen}>
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Remove this contact from <span className="font-semibold">{selectedEnrollment.program.name}</span>?
                  <br /><br />
