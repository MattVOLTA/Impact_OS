/**
 * Pending Queue Table Component (Tab 1)
 *
 * Displays meetings awaiting review with:
 * - Smart badges (matched contacts, active support)
 * - Bulk selection and import/exclude
...
import { Clock, Users, X, Eye, Check } from 'lucide-react'
import { toast } from 'sonner'
import { importSelectedMeetings, excludeSelectedMeetings, excludeStagedMeeting } from '../../../actions'
...
interface StagedMeeting {
  ...
  matched_contact_names?: Array<{ email: string; name: string }>
...
export function PendingQueueTable({ meetings }: PendingQueueTableProps) {
  ...
  const handleBulkExclude = async () => {
    if (selectedIds.size === 0) {
      toast.error('Please select at least one meeting to exclude')
      return
    }
    ...
    try {
      const result = await excludeSelectedMeetings(Array.from(selectedIds))

      if (result.success) {
        toast.success(result.message)

        // Remove excluded meetings from local state
        setLocalMeetings(localMeetings.filter(m => !selectedIds.has(m.id)))
        setSelectedIds(new Set())
      ...
      toast.error('Failed to exclude meetings')
    } finally {
      setIsExcluding(false)
    }
  }

  const handleExcludeOne = async (meetingId: string, title: string) => {
  ...
  const handleImportOne = async (meetingId: string, title: string) => {
    setIsImporting(true)

    try {
      const result = await importSelectedMeetings([meetingId])
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 border rounded-lg">
        <div className="text-center space-y-2">
          <h3 className="text-lg font-medium">No pending meetings</h3>
          <p className="text-sm text-muted-foreground">
            Run a sync to fetch new meetings from Fireflies, or check the Imported/Excluded tabs
          </p>
        </div>
      </div>
    )
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Button
          ...
          disabled={selectedIds.size === 0 || isExcluding}
          size="sm"
        >
          {isExcluding
            ? `Excluding ${selectedIds.size}...`
          ...
          <TableBody>
            {localMeetings.map(meeting => (
              <TableRow key={meeting.id}>
                ...
                <TableCell>
                  <MatchBadge matchType={meeting.match_type} contactNames={meeting.matched_contact_names} />
                </TableCell>
                <TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleImportOne(meeting.id, meeting.title)}
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleExcludeOne(meeting.id, meeting.title)}
                      disabled={excludingId === meeting.id}
                      title="Exclude This Meeting"
                    >
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
