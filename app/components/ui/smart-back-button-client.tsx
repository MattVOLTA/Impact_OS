/**
 * Smart Back Button (Client Component)
 *
 * Client-side version of SmartBackButton that receives referer as a prop.
...
interface SmartBackButtonClientProps {
  referer: string | null
  fallbackHref: string
 ...
 */
function getBackContext(referer: string | null, fallbackHref: string, fallbackLabel: string) {
  if (!referer) {
...
export function SmartBackButtonClient({
  referer,
  fallbackHref,
  fallbackLabel = 'Back'
}: SmartBackButtonClientProps) {
  const { label, href } = getBackContext(referer, fallbackHref, fallbackLabel)
